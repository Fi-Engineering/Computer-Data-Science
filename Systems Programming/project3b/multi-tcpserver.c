...
typedef struct serverMeta {
  struct sockaddr_in sin;
  int s;
} serverMeta;

serverMeta* initMeta(struct sockaddr_in sin, int s) {
  serverMeta* meta = (serverMeta*) malloc(sizeof(serverMeta));
  assert(meta != NULL);
  meta->sin = sin;
  meta->s = s;
  return meta;
...
int main(int argc, char *argv[]) {
  ...
  pthread_t tids[10];
  serverMeta* meta = initMeta(sin, s);

  for (int i=0; i<10; i++) {
    pthread_create(&tids[i], NULL, waitForClient, (void *)meta);
  }
...
void *waitForClient(void *arg) {
  serverMeta* meta = (serverMeta*) arg;
  struct sockaddr_in sin = meta->sin;
  int s = meta->s;
